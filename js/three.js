            ...
            var skyMat = new THREE.MeshBasicMaterial( { color: 0x111111 } );
            ...
            var shipG = new THREE.SphereGeometry(0.8, 68, 2);
            var shipM = new THREE.MeshPhongMaterial({
                color: 0xff0011
            });

            var myShip = new THREE.Mesh(shipG, shipM);
            


                myShip.position.z = -4;
                myShip.position.y = -1;
                myShip.rotation.x = 0.7;

            document.getElementById("ready").addEventListener("click",controlShip)

            function controlShip(){
                myShip.position.z = -2;
                myShip.position.y = ZZ + 1;
                myShip.rotation.x = 0;
                myShip.rotation.y = Math.PI;
...
function handleTouchMove(evt) {
    ...
    if ( Math.abs( xDiff ) > Math.abs( yDiff ) ) {/*most significant*/
        if ( xDiff > 0  && myShip.position.x != -3 && myShip.position.x >= -2 ) {
            /* right swipe */ 
            
            let i =  myShip.position.x;
            let I =  myShip.position.x - 3;
            function ML(){
                i-=0.15
                    myShip.position.x = i
                    if(i <= I){
            ...
            /* left swipe */
            if(myShip.position.x != 3 && myShip.position.x <= 2){
            let i =  myShip.position.x;
                    let I =  myShip.position.x + 3;
                    function MR(){
                            i+=0.15
        ...
        if ( yDiff > 0  && myShip.position.y <= ZZ+2) {
            /* down swipe */ 
            let i =  myShip.position.y;
            let I =  myShip.position.y + 2;
            function MD(){
                    i+=0.15
                    myShip.position.y = i
                    if(myShip.position.y >= I){
                        clearInterval(SId);
                    }
            }
    ...
    yDown = null;                                             
      ...
      //end of mobile version2
                ...
                if (keyPressed === LEFT_KEY && myShip.position.x != -3 && myShip.position.x >= -2) {
                    let i =  myShip.position.x;
                    let I =  myShip.position.x - 3;
                    function ML(){
                        myShip.rotation.z -= 0.08;
                        i-=0.15
                            myShip.position.x = i
                            if(i <= I){
                                function rl(){
                                    myShip.rotation.z += 0.02;
                                    if(myShip.rotation.z >= 0){
                                        clearInterval(RL);
                                    }
                                }
                ...
                if (keyPressed === UP_KEY && myShip.position.y <= ZZ+2) {
                    let i =  myShip.position.y;
                    let I =  myShip.position.y + 2;
                    function MD(){
                            myShip.rotation.x += 0.06;
                            i+=0.15
                            myShip.position.y = i
                            if(myShip.position.y >= I){
                                function ru(){
                                    myShip.rotation.x -= 0.02;
                                    if(myShip.rotation.x <= 0){
                                        clearInterval(RU);
                                    }
                                }
                ...
                if (keyPressed === RIGHT_KEY && myShip.position.x != 3 && myShip.position.x <= 2) {
                    // myShip.position.x += 3;
                    let i =  myShip.position.x;
                    let I =  myShip.position.x + 3;
                    function MR(){
                ...
                if (keyPressed === DOWN_KEY && myShip.position.y >= ZZ+2) {
                    let i =  myShip.position.y;
                    let I =  myShip.position.y -2;
                    function MD(){
                            myShip.rotation.x -= 0.06;
            ...
            function detectCollisionCubes(){
                ...
                if(myShip.position.y > ZZ+3){
                    myShip.position.y = ZZ+3
                }else if(myShip.position.y < ZZ+1){
                    myShip.position.y = ZZ+1
                }else if(myShip.position.x > 3){
                    myShip.position.x = 3
                }else if(myShip.position.x < -3){
                    myShip.position.x = -3
                }
                for(i=0 ; i<allSpheres.length - 1 ; i++){
                    if(allSpheres[i].position.distanceToSquared(myShip.position) <= 1.5){
